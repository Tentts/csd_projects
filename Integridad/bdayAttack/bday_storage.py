...
import sys
import time
## Usage : python3 <dictionaryFile> <hashSelector> <hashBits> 
## Possible characters.
...
def animate( FunctionStr = ""):
    ...
    for c in itertools.cycle(['|', '/', '-', '\\']):
        if DictionaryDone:
            ...
            sys.stdout.write(f"\rWorking on {FunctionStr}....................{c}")
            sys.stdout.flush()
            time.sleep(0.05)
...
def createMs(originalString):
    ...
    while (it_created < number_of_Ms):
        ...
        if(hss not in dictionary):
            ...
            it_created +=1
            # if(it_created%1000 == 0):
            #     print("Created -\t"+ str(it_created)+"\t\t"+ str(number_of_Ms - it_created)+ " to go.")
            #print(hss)
            #print(it_created)
    #print("Generation of password dictionary done.")
    return dictionary
...
if(len(sys.argv) > 1):
    ...
    print(f"ERROR. INSUFFICIENT CLI ARGUMENTS. USAGE:\n\t python3 slammer_mutable.py <dictionary_path> <hash_to_use> <n_hash_bits>\nhash_to_use:\n\t· 0 - crc32\t- max 32 bits\n\t· 1 - md4\t- max 128 bits\n\t· 2 - md5\t- max 128 bits\n\t· 3 - sha1\t- max 160 bits\n")
...
if(os.path.exists(fpath)):
    print(f"File {fpath} detected.\nMake sure the dictionary you have chosen has been built using the same hash algorithm.\nLoading dictionary...")
    ...
    MDictionary = dict([])
    for row in tmpDict:
        MDictionary[bin(int(row, 10))[2:]] = tmpDict[row]
    ...
    while(ans not in 'YyNn'):
        ...
        if(ans in 'Nn' and ans not in ''):
            print("Fatal error. Execution is not able to continue without creating dictionary.")
            exit(0)
        
    print(f"Generating {fpath}...")
    #print('Generating password dictionary from string:\n\''+ M+'\n')
    print(f"Generating {number_of_Ms} combinations...")
    GenStartTime = time.time()
    
    t = threading.Thread(target=animate,daemon=True, name="thread_animation", args=["dictionary generation",])
    t.start()
    MDictionary = createMs(M)
    
    fichero = open(fpath, "w")
    csvW = csv.writer(fichero)
    for key_ , value_ in MDictionary.items():
        csvW.writerow([int(key_,base=2), value_])
    DictionaryDone = True
    
    print(f"{fpath} generation done.")
    GenTotalTime = time.time() - GenStartTime
    print("Took "+ str(GenTotalTime)+" seconds.")
    fichero.close()
...
while( ( collision == False ) and EvilIterations < Max_iterations_for_collision):
    ...
    EvilIterations+=1
    #if(EvilIterations%1000 == 0):
        #print("Iteration - "+ str(EvilIterations)+"\tCollision: "+str(collision)+"\tX: "+Xi + "\tM:"+M)
...
if(collision):
    ...
    print(f"Took {EvilIterations} iterations. ({elapsedTime} seconds)")
    print(f"XHash:\t{XHash}")
