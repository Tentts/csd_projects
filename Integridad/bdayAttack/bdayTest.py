...
print(f"Launching {max_its} test iterations.")

startTime = time.time()

totalElapsedTime = totalSearchTime = totalGenTime = 0
iterador = 0
while iterador < max_its:
    collision = False
    qTime = time.time()
    MDictionary = createMs(M)
    GenTime = time.time() - qTime
    EvilIterations = 0
    Xi = X
    variationsX = list()
    ...
    while( ( collision == False ) and EvilIterations < Max_iterations_for_collision):
        ...
        if( XHash in MDictionary ):
            ...
            Xi,variationsX = mutateString(X)
        
        EvilIterations+=1
    elapsedTime = time.time() - qTime
    searchTime = elapsedTime - GenTime
...
AvgGenTime =  totalGenTime/iterador
AvgSearchTime = searchTime/iterador
AvgElapsedTime = totalElapsedTime/iterador

print("====================================== END ======================================")

print(f"Done {iterador} collisions.")
print(f"Average generation time: {AvgGenTime} seconds.")
print(f"Average search time: {AvgSearchTime} seconds.")
print(f"Average elapsed time: {AvgElapsedTime } seconds.")
